...
double _Game::DiemTanCongDuyetDoc(int i, int j)
{
	...
	int SoQuanTa = 0;
	int SoQuanDich = 0;
	for (int dem = 1; dem < 7 && i + dem < _b->getSize(); dem++) //Duyệt mảng từ trên xuống bắt đầu từ ô cờ đang xét
	{
	...
	for (int dem = 1; dem < 7 && i - dem >= 0; dem++) //Duyệt mảng từ dưới lên bắt đầu từ ô cờ đang xét
	{
		if (_b->getpArr(i - dem, j) == 1)
			SoQuanTa++;
		else if (_b->getpArr(i - dem, j) == -1)
...
double _Game::DiemTanCongDuyetNgang(int i, int j)
{
	...
	int SoQuanTa = 0;
	int SoQuanDich = 0;
	for (int dem = 1; dem < 7 && j + dem < _b->getSize(); dem++) // Duyệt mảng từ trái sang phải bắt đầu từ ô cờ đang xét
	{
	...
	for (int dem = 1; dem < 7 && j - dem >= 0; dem++) // Duyệt mảng từ phải sang trái bắt đầu từ ô cờ đang xét
	{
		if (_b->getpArr(i, j - dem) == 1)
			SoQuanTa++;
		else if (_b->getpArr(i, j - dem) == -1)
...
double _Game::DiemTanCongDuyetCheoNguoc(int i, int j)
{
	...
	int SoQuanTa = 0;
	int SoQuanDich = 0;
	for (int dem = 1; dem < 7 && i - dem >= 0 && j + dem < _b->getSize(); dem++) // Chéo ngược đi lên 
	{
	...
	for (int dem = 1; dem < 7 && i + dem < _b->getSize() && j - dem >= 0; dem++) // Chéo ngược đi xuống
	{
		if (_b->getpArr(i + dem, j - dem) == 1)
			SoQuanTa++;
		else if (_b->getpArr(i + dem, j - dem) == -1)
...
double _Game::DiemTanCongDuyetCheoXuoi(int i, int j)
{
	...
	int SoQuanTa = 0;
	int SoQuanDich = 0;
	for (int dem = 1; dem < 7 && i + dem < _b->getSize() && j + dem < _b->getSize(); dem++) //Chéo xuôi đi xuống
	{
		if (_b->getpArr(i + dem, j + dem) == 1)
			SoQuanTa++;
		else if (_b->getpArr(i + dem, j + dem) == -1)
	...
	for (int dem = 1; dem < 7 && i - dem >= 0 && j - dem >= 0; dem++) //Chéo xuôi đi lên
	{
		if (_b->getpArr(i - dem, j - dem) == 1)
			SoQuanTa++;
		else if (_b->getpArr(i - dem, j - dem) == -1)
...
double _Game::DiemPhongNguDuyetDoc(int i, int j)
{
	...
	for (int dem = 1; dem < 7 && i + dem < _b->getSize(); dem++)
	{
		if (_b->getpArr(i + dem, j) == 1)
		{
			SoQuanTa++;
	...
	for (int dem = 1; dem < 7 && i - dem >= 0; dem++)
	{
		if (_b->getpArr(i - dem, j) == 1)
		{
			SoQuanTa++;
	...
	if ((temp1 >= 3 && temp2 >= 1) || (temp2 >= 3 && temp1 >= 1) || (SoQuanDich == 4 && SoQuanTa == 0))
	{
		DiemTong += MangDiemPhongNgu[SoQuanDich + 1] * 4;
	}
...
double _Game::DiemPhongNguDuyetNgang(int i, int j)
{
	...
	for (int dem = 1; dem < 7 && j + dem < _b->getSize(); dem++)
	{
		if (_b->getpArr(i, j + dem) == 1)
		{
			SoQuanTa++;
	...
	for (int dem = 1; dem < 7 && j - dem >= 0; dem++)
	{
		if (_b->getpArr(i, j - dem) == 1)
		{
			SoQuanTa++;
...
double _Game::DiemPhongNguDuyetCheoNguoc(int i, int j)
{
	...
	int temp1 = 0; //quân địch bên trái
	int temp2 = 0; //Quân địch bên phải
	for (int dem = 1; dem < 7 && i - dem >= 0 && j + dem < _b->getSize(); dem++)
	{
		if (_b->getpArr(i - dem, j + dem) == 1)
		{
			SoQuanTa++;
	...
	for (int dem = 1; dem < 7 && i + dem < _b->getSize() && j - dem >= 0; dem++)
	{
		if (_b->getpArr(i + dem, j - dem) == 1)
		{
			SoQuanTa++;
...
double _Game::DiemPhongNguDuyetCheoXuoi(int i, int j)
{
	...
	int temp1 = 0; //quân địch bên trái
	int temp2 = 0; //Quân địch bên phải
	for (int dem = 1; dem < 7 && i + dem < _b->getSize() && j + dem < _b->getSize(); dem++)
	{
		if (_b->getpArr(i + dem, j + dem) == 1)
		{
			SoQuanTa++;
	...
	for (int dem = 1; dem < 7 && i - dem >= 0 && j - dem >= 0; dem++)
	{
		if (_b->getpArr(i - dem, j - dem) == 1)
		{
			SoQuanTa++;
